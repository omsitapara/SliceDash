...
<div class="row">
	...
	<div class="col-md-6 col-lg-3 grid-margin stretch-card">
	  <div class="card">
		<div class="card-body">
		  <h1 class="font-weight-light mb-4">
			<?php 
			$row=mysqli_fetch_assoc(mysqli_query($con,"select count(order_master.user_id) as t,user.name from order_master,user WHERE order_master.user_id=user.id and payment_status='success' GROUP BY order_master.user_id order by count(order_master.user_id) desc limit 1"));
			echo $row['name'];
			echo "<br/>";
			echo '<span style="font-size:15px;">('.$row['t'].' Times)</span>';
			?>
  ...
  <?php
  $sql="select order_master.*,order_status.order_status as order_status_str from order_master,order_status where order_master.order_status=order_status.id and payment_status='success' order by order_master.id desc limit 5";
			...
			<table class="table table-hover">
			  ...
			  <tbody>
						...
						<tr>
							...
							<td style="font-size:14px;"><?php echo '₹'.$row['final_price']?><br/>
							</td>
